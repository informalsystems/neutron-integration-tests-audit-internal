...
describe('TGE / Migration / PCL contracts', () => {
  ...
  describe('Pre-Migration setup', () => {
      ...
      describe('Phase 3', () => {
        ...
        describe('lock_lp', () => {
          it('should be able to lock ATOM LP tokens', async () => {
            let data: Map<string, InitAmounts> = initialStateData.stepInfo.msgArgs.value
            for (const key of data.keys()) {
              if(!initialStateData.users.get(key)?.only_vesting){
                console.log(`${key} user is not only vesting`);
                ...
                console.log(`${key} user is only vesting`);
              }
    ...
    describe('Migrate to new contracts', () => {
      ...
      describe('migrate TGE contracts to liquidity migration versions', () => {
        ...
        it('should migrate USDC LP vesting to V2', async () => {
          const res = await cmInstantiator.migrateContract(
  ...
  describe('Quint generated steps', () => {
    ...
    var stateIndex = 0;
    var isFirstPostMigrationOperation = true;
      ...
      switch(state.stepInfo.actionTaken){
        ...
        case 'migrate': {
          describe(`Quint generated step MIGRATE from step ${state.numSteps}`, () => {
            ...
            if(!state.users.get(userAddress)?.only_vesting){
              ...
              describe(`execute migration of vesting for user ${userAddress}`, () => {
                it('should validate numbers & save claim amount before migration', async () => {
                  const [
                    ...
                    neutronChain.queryContract<VestingAccountResponse>(
                      ...
                      {
                        vesting_account: {
                          address: tgeWallets[userAddress].wallet.address.toString(),
                        },
                      },
                    ),
                    neutronChain.queryContract<VestingAccountResponse>(
                    ...
                    const resAtom = await cmInstantiator.executeContract(
                      ...
                      JSON.stringify({
                        migrate_liquidity_to_pcl_pool: {
                          user: tgeWallets[userAddress].wallet.address.toString(),
                        },
                      }),
                    );
                ...
                it('should claim', async () => {
                  ...
                  const resAtom = await tgeWallets[userAddress].executeContract(
                    atomVestingLpAddr,
                    JSON.stringify({
                      claim: {},
                    }),
                  ...
                  expect(resAtom.code).toEqual(0);
                  const resUsdc = await tgeWallets[userAddress].executeContract(
                    usdcVestingLpAddr,
                    JSON.stringify({
                      claim: {},
                    }),
        ...
        case 'claim_rewards_pcl': {
          describe(`Quint generated step CLAIM_REWARDS_PCL from step ${state.numSteps}`, () => {
            ...
            let success = state.stepInfo.actionSuccessful;
            let i = stateIndex
            let stateToGetPCLData = stateIndex === 0 ? initialStateData : otherStatesData[--i];
            let error_msg = stateToGetPCLData.users.get(sender)?.pcl_liquidity_withdrawn ? /Astro LP Tokens have already been claimed!/ : /No rewards available to claim!/;

            if(isFirstPostMigrationOperation){
            ...
            describe(`user ${sender} testing post migration state`, () => {
                ...
                describe(`${sender} claim without withdrawal`, () => {
                  if(success){
                    ...
                    describe('funds flow', () => {
                      ...
                      // @audit fix this here; if first time, then be equal, if not, then error
                      it('no ntrn received by the user', async () => {
                        ...
                        expect(stateAfter.balances.user.ntrn).toEqual(
                          stateBefore.balances.user.ntrn - ntrnToPayGas * 2, // fees for 5 withdrawal/claim attempts
                        );
                      });
                    });
                  }else{
          ...
          isFirstPostMigrationOperation = false;
          break;
        };
        default: {
        }
